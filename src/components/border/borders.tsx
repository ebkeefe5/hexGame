...
export default function Border({ borderNumber, boardDimension }: BorderProps): JSX.Element | null {
  ...
  const y1 = TOP_LEFT_HEXAGON_CENTER_Y - 0.5 * HEXAGON_EDGE_LENGTH;
  const y2 = TOP_LEFT_HEXAGON_CENTER_Y - HEXAGON_EDGE_LENGTH;
  const y3 = TOP_LEFT_HEXAGON_CENTER_Y - 1.5 * HEXAGON_EDGE_LENGTH;
  ...
  const y4 = TOP_LEFT_HEXAGON_CENTER_Y + (boardDimension * 1.5 - 1) * HEXAGON_EDGE_LENGTH;
  const y5 = TOP_LEFT_HEXAGON_CENTER_Y + (boardDimension * 1.5 - 0.5) * HEXAGON_EDGE_LENGTH;
  ...
  const y6 = TOP_LEFT_HEXAGON_CENTER_Y + (boardDimension * 1.5) * HEXAGON_EDGE_LENGTH;
  ...
  if (borderNumber === 0) {
    ...
    return <polygon points={points} fill={COLORS[1]} />;
  }

  if (borderNumber === 1) {
    const points = `${x3},${y1} ${x4},${y3} ${x7},${y4} ${x6},${y5} ${x5},${y4}`;
    return <polygon points={points} fill={COLORS[2]} />;
  }
  ...
  return null; // Fallback for unsupported borderNumber
}